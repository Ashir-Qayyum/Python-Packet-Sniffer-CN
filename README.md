...
*(The clean, initial state of the GUI before packet capture begins.)*
...
*(The application capturing UDP packets in real-time, with each packet color-coded for easy identification in the "Capture Feed".)*
...
*(Demonstrates the sniffer capturing HTTP and TCP packets generated from browsing a website in the background.)*
...
*(Cross-validating the captured destination IP of an HTTP packet with the result of a manual `nslookup` command, confirming accuracy.)*
...
*(The protocol filter has been applied to show only HTTP packets, hiding all other protocol types from the main view.)*
...
*(The "Packet Details" window is open, showing the parsed layer 4 (TCP) and layer 7 (HTTP) header information for a selected packet.)*
...
*(The detailed view for a selected TCP packet, showing its header fields, flags, and the raw TCP data payload.)*
...
*(The detailed view for a selected UDP packet, displaying its header information and raw data payload.)*
...
*(The filter is set to "OTHER," correctly isolating a packet using IGMP (Protocol 2) and displaying its basic IP and raw data.)*
...
*(The `capture.pcap` file generated by the sniffer is opened in Wireshark, verifying that the output is valid and compatible with professional analysis tools.)*
